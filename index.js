...
function isTooLarge(num) {
  ...
  const isMax = num > Number.MAX_VALUE;
  return isMax;
}
...
function isAlmostZero(num) {
  ...
  const isMin = num > 0 && num < Number.MIN_VALUE;
  return isMin;
}
...
function checkIsInteger(num) {
  ...
  const ifIsInteger = Number.isInteger(num);
  return ifIsInteger;
}
...
function checkIsSafeInteger(num) {
  ...
  const safeInt = Number.isSafeInteger(num);
  return safeInt;
}
...
function convertToFixed(num, precision) {
  ...
  const fix = num.toFixed(precision);
  return fix;
}
...
function convertToPrecision(num, precision) {
  ...
  const prec = num.toPrecision(precision);
  return prec;
}
